...
// Copyright (c) 2017 The Crypto4Like developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/crypto4like-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("crypto4like-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "crypto4like.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "crypto4liked.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the CRYPTO4LIKE and zC4L money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 53388, 5520));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in CRYPTO4LIKE/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in CRYPTO4LIKE/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, crypto4like, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in CRYPTO4LIKE/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Crypto4Like specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:53388"));
    ...
    strUsage += HelpMessageOpt("-backupzcrypto4like=<n>", strprintf(_("Enable automatic wallet backups triggered after each zC4L minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizecrypto4likeamount=<n>     " + strprintf(_("Keep N CRYPTO4LIKE anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Crypto4Like Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("crypto4like-loadblk");
...
/** Sanity checks
 *  Ensure that Crypto4Like is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Crypto4Like Core is shutting down."));
...
#endif
    // Make sure only a single Crypto4Like process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Crypto4Like Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Crypto4Like version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Crypto4Like: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZCRYPTO4LIKEMinted();
                        RecalculateZCRYPTO4LIKESpent();
                    }
                    RecalculateCRYPTO4LIKESupply(1);
                }
                ...
                // Crypto4Like: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Crypto4Like Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Crypto4Like Core to complete") << "\n";
        ...
        bool fEnableZCrypto4LikeBackups = GetBoolArg("-backupzcrypto4like", true);
        pwalletMain->setZCrypto4LikeAutoBackups(fEnableZCrypto4LikeBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeCrypto4LikeAmount = GetArg("-anonymizecrypto4likeamount", 0);
//    if (nAnonymizeCrypto4LikeAmount > 999999) nAnonymizeCrypto4LikeAmount = 999999;
//    if (nAnonymizeCrypto4LikeAmount < 2) nAnonymizeCrypto4LikeAmount = 2;
    ...
    LogPrintf("Anonymize Crypto4Like Amount %d\n", nAnonymizeCrypto4LikeAmount);
    ...
    /* Denominations
       ...
       For example:
       1CRYPTO4LIKE+1000 == (.1CRYPTO4LIKE+100)*10
       10CRYPTO4LIKE+10000 == (1CRYPTO4LIKE+1000)*10
    */
