...
#include "zcrypto4likecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zC4L ought to be enough for anybody." - Bill Gates, 2017
    ui->zC4LpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzC4LSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzC4L->setEnabled(false);
        ui->pushButtonMintzC4L->setToolTip(tr("zC4L is currently disabled due to maintenance."));

        ui->pushButtonSpendzC4L->setEnabled(false);
        ui->pushButtonSpendzC4L->setToolTip(tr("zC4L is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zC4LpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzC4L_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zC4L is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zC4L...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zC4L in ") +
...
void PrivacyDialog::on_pushButtonSpendzC4L_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zC4L is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zC4L
        sendzC4L();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zC4L
    sendzC4L();
}

void PrivacyDialog::on_pushButtonZCrypto4LikeControl_clicked()
{
    ZCrypto4LikeControlDialog* zC4LControl = new ZCrypto4LikeControlDialog(this);
    zC4LControl->setModel(walletModel);
    zC4LControl->exec();
}

void PrivacyDialog::setZCrypto4LikeControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzC4LSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzC4L()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Crypto4Like Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zC4LpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zC4L is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CRYPTO4LIKE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zC4L</b>";
    ...
    // use mints from zC4L selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCrypto4LikeControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCrypto4LikeControlDialog::GetSelectedMints();
    }
    ...
    // Clear zcrypto4like selector in case it was used
    ZCrypto4LikeControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zC4L Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Crypto4Like, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zC4L Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zC4L </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zC4L "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zC4L "));
    ui->labelzC4LAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzC4LSyncStatus->setVisible(fShow);
}
