...
#include "ui_zcrypto4likecontroldialog.h"
...
std::list<std::string> ZCrypto4LikeControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZCrypto4LikeControlDialog::listMints;

ZCrypto4LikeControlDialog::ZCrypto4LikeControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZCrypto4LikeControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZCrypto4LikeControlDialog::~ZCrypto4LikeControlDialog()
{
    delete ui;
}

void ZCrypto4LikeControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCrypto4LikeControlDialog::updateList()
{
...
void ZCrypto4LikeControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCrypto4LikeControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZCrypto4Like_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCrypto4LikeControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZCrypto4LikeControlDialog::GetSelectedMints()
{
...
void ZCrypto4LikeControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
